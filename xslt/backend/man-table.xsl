...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- DB2X CHANGE: dbhtml-attribute -->
  <xsl:template name="dbhtml-attribute">
    <xsl:param name="pis" />
    <xsl:param name="attribute" />
  </xsl:template>

  <!-- DB2X CHANGE: HTML parameters -->
  ...
  <!-- DB2X CHANGE: anchor template -->
  <xsl:template name="anchor" />

  <!-- DB2X CHANGE: library templates -->
<xsl:template name="copy-string">
  ...
  <!-- DB2X CHANGE: the very first table template -->
